...
public class ThongKeDAO {
    ...
    public static ArrayList<ThongKeTonKhoDTO> getThongKeTonKho(String txt, Date start_day , Date end_day){
       ArrayList<ThongKeTonKhoDTO> result = new ArrayList<>();
        
        try {
            
            Calendar calendar = Calendar.getInstance();
            calendar.setTimeInMillis(end_day.getTime());
            ...
            String sql = """
                             WITH nhap AS (
                                     		...
                                     		FROM ctphieunhap AS ctpn, phieunhap AS pn
                                     		WHERE pn.maphieunhap = ctpn.maphieunhap AND pn.thoigian between ? AND ?
                                     		GROUP BY maphienbansp),
                                     		xuat AS  (
                                     		SELECT maphienbansp , SUM(soluong) AS sl_xuat
                                     		FROM ctphieuxuat AS ctpx , phieuxuat AS px
                                     		WHERE ctpx.maphieuxuat = px.maphieuxuat AND px.thoigian between ? AND ?
                                     		GROUP BY maphienbansp
                                     		),
                                     		nhap_dau AS (
                                     		SELECT ctpn.maphienbansp , SUM(ctpn.soluong) AS sl_nhap_dau
                                     		FROM ctphieunhap AS ctpn, phieunhap AS pn
                                     		WHERE pn.maphieunhap = ctpn.maphieunhap AND pn.thoigian < ?
                                     		GROUP BY ctpn.maphienbansp
                                     		),
                                     		 xuat_dau AS (
                                     		SELECT maphienbansp , SUM(soluong) AS sl_xuat_dau
                                     		FROM ctphieuxuat AS ctpx , phieuxuat AS px
                                     		WHERE ctpx.maphieuxuat = px.maphieuxuat AND px.thoigian < ?
                                     		GROUP BY maphienbansp
                                     		),
                                     		dau_ky AS (
                                     		SELECT
            ...
            pst.setTimestamp(1,new Timestamp(start_day.getTime()));
            pst.setTimestamp(2,new Timestamp(calendar.getTimeInMillis()));
            pst.setTimestamp(3,new Timestamp(start_day.getTime()));
            pst.setTimestamp(4,new Timestamp(calendar.getTimeInMillis()));
            pst.setTimestamp(5,new Timestamp(start_day.getTime()));
            pst.setTimestamp(6,new Timestamp(start_day.getTime()));
            pst.setString(7, "%" + txt + "%");
            pst.setString(8, "%" + txt + "%");
    ...
    public static ArrayList<ThongKeDoanhThuDTO> getThongKeTheoTungNam(int year_start , int year_end){
        ArrayList<ThongKeDoanhThuDTO> result = new ArrayList<>();
